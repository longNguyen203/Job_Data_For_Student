...
"""
 - Thư viện urllib.parse trong Python cung cấp các hàm để phân tích các thành phần của một URL 
 và tạo các chuỗi query parameter.
 - Hàm urlencode được sử dụng để tạo chuỗi query parameter từ một từ điển (dictionary) hoặc dãy (sequence) các cặp key-value. 
 Nó thực hiện việc mã hóa các giá trị này thành một chuỗi dạng key1=value1&key2=value2&..., phù hợp để thêm vào URL.
"""
...
class JobitscraperSpiderMiddleware:
...
class JobitscraperDownloaderMiddleware:
    # Not all methods need to be defined. If a method is not defined,
